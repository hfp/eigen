...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, ThreadPoolDevice> :
  ...
  void evalProduct(Scalar* buffer) const {
    ...
    if (NULL != this->m_xgemm_handle) {
      if (this->m_device.numThreads() == 1) {
...
#undef CONTEXT_ARGS
 ...
 template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous,
  ...
  class ContextXsmm {
   public:
    ContextXsmm(const Self* self, Scalar* buffer):
      handle(self->m_xgemm_handle),
      workers_done(self->m_device.numThreads()),
      device(self->m_device),
      a(self->m_leftImpl.data()),
      b(self->m_rightImpl.data()),
      scratch(self->m_xgemm_scratch),
      c(buffer)
    {}
